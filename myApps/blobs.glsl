...
vec2 makeSymmetry(vec2 p){
   vec2 ret=p;
   ret=rotsim(ret,sin(iGlobalTime*0.3)*2.0+3.0);
...
void main( void ) {
   vec2 p = gl_FragCoord.xy/iResolution.y-vec2((iResolution.x/iResolution.y)/2.0,0.5);
   p=rot(p,sin(iGlobalTime+length(p))*4.0);
   p=zoom(p,sin(iGlobalTime*2.0)*0.5+0.8);
   ...
   float t=iGlobalTime*iOvertoneVolume;
   ...
   vec3 d=vec3(a,b,c)*iOvertoneVolume/4.0;
